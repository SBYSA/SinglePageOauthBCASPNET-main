...
namespace SBLawyerWebApplication.Helpers
{
    public static class CultureHelper
    {
        ...
        // Cultures implémentées
        private static readonly List<string> _cultures = new List<string> {
        "fr-FR",  // La première est celle par défaut
        "en", // Culture anglaise neutre
        
    };
       
        /// <summary>
        /// Retourne la culture du en fonction du nom donné en paramètre. Si elle n'est pas valide, on récupère le "fr-FR"
        /// </summary>
        /// <param name="name" />nom de la culture (e.g. en-US)</param>
        public static string GetImplementedCulture(string name)
        {
            // Non null
            if (string.IsNullOrEmpty(name))
                return GetDefaultCulture(); // Culture par défaut
            // Culture valide
            if (_validCultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() == 0)
                return GetDefaultCulture(); // Culture ar défaut
            // Culture implémentée
            if (_cultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
                return name; // accept it
            // Trouve une culture proche. Par exemple, si on a "fr-FR" de défini et qu'on demande "fr-BE", 
            // on retourne la valeur proche "fr-FR" car on a au moins le langage  
        ...
        /// <summary>
        /// Returns default culture name which is the first name decalared (e.g. fr-FR)
        /// </summary>
        /// <returns></returns>
        ...
        public static string GetNeutralCulture(string name)
        {
            ...
            return name.Split('-')[0]; // Read first part only. E.g. "fr", "en"
        }
    }
}